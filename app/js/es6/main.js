...
new Vue({
  ...
  methods: {
    ...
    postToZsDesserts() {
      ...
      //   if (data.code == 200) {
      window.presentUrl =
        "https://github.com/tylerrrkd?desId=" + "xsj" + "&dessert=" + this.dimSumSelected;
      window.shareTitle = "兄弟，给你看个宝贝！";
      window.shareDes =
        "一抽999，道具能换钱，" + this.userInfo.name + "也在玩，你还等什么！";
      this.isShareShow = true;
      // window.friendShare((data) => {
      // if (data > 0) {
      this.isShareShow = false;
      this.tips.content =
        "成功赠出一只点心！<br>好友领取后，你将获得一次投骰子机会";
      this.tips.isTipsShow = true;
      // } else {
      //   this.isShareShow = false;
  ...
  computed: {
    getPhoneEncrypt() {
      ...
      return (
        this.initUser.phone.substr(0, 3) +
        "****" +
        this.initUser.phone.substr(7, 11)
      );
    },
    dessertList() {
      if (Object.keys(this.initUser).length > 0) {
        let rawKeys = Object.keys(this.initUser).filter((v) =>
          /dessert([0-9]+)/.test(v)
        );
